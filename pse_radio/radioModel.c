...
ppmNetHandle ppi_notification_handle;
...
static void busPortConnections(void) {
    ...
    radio_window = ppmCreateSlaveBusPort("RADIO", 0x1000); 
    
    if (!radio_window) {
        bhmMessage("E", "PPM_SPNC", "Could not connect port 'radio_window'");
    }

    createRegisters(radio_window);
}
...
static void netPortConnections(void) {
    irq_handle  = ppmOpenNetPort("radio_irq");
    ppi_notification_handle = ppmOpenNetPort("radio_ppi");
}
